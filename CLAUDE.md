...
```bash
bun run --filter web dev          # Dev server on port 3000
bun run --filter web build        # Build web app only
bun run --filter @repo/ui lint    # Lint UI package only
```
...
- **`apps/web`** — Next.js 16 app (React 19). The main web application. Uses App Router.
- **`packages/ui`** (`@repo/ui`) — Shared component library built on Radix UI + Tailwind CSS 4 + Class Variance Authority. Contains:
...
- **`packages/eslint-config`** (`@repo/eslint-config`) — Shared ESLint flat configs (base, Next.js, React)
- **`packages/typescript-config`** (`@repo/typescript-config`) — Shared tsconfig bases

## Key Patterns

- **Imports from `@repo/ui`** use subpath exports: `@repo/ui/web/primitives/button`, `@repo/ui/utils/cn`, `@repo/ui/tokens`, `@repo/ui/web/hooks/use-media-query`
- **Styling**: Tailwind CSS 4 with CSS-first config. Theme is defined via CSS variables (HSL) in `packages/ui/src/web/styles/globals.css`. The web app's `globals.css` imports `@repo/ui/globals.css`.
